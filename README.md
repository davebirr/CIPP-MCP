...
> - [Example converstation with CIPP-MCP test tool - Monkey](https://claude.ai/share/385e67b9-7e4c-48f3-9289-57a5773ec691)
...
- New Docker / Azure Container App alongside existing function app. The MCP server will behave like the CIPP SWA as far as the API is concerned.
- **Language:** C# (.NET)
...
- The project requires an explicit reference to `Microsoft.Extensions.Hosting` version `9.0.0-preview.3.24172.9` in the `.csproj` file to ensure compatibility with .NET 9 and to resolve build issues with `Host.CreateApplicationBuilder`.

## Installing Preview AI Schema Support

To enable advanced JSON schema customization for MCP tools, install the preview Microsoft.Extensions.AI package:

```sh
dotnet add package Microsoft.Extensions.AI --prerelease
```

This provides access to `AIJsonSchemaCreateOptions` and related types for schema transformation.

## Running and Writing Tests

This project uses [xUnit](https://xunit.net/) for unit and integration testing.

To add xUnit to your test project, run:

```sh
dotnet add package xunit
```

For Visual Studio or `dotnet test` integration, also add:

```sh
dotnet add package xunit.runner.visualstudio
```

You can now write tests using `[Fact]` and `[Theory]` attributes in your test classes.
...
If your tests or server require Newtonsoft.Json, add it to your test project with:

```sh
dotnet add package Newtonsoft.Json --version 13.0.3
```

This will automatically update your .csproj file with:

```xml
<PackageReference Include="Newtonsoft.Json" Version="13.0.3" />
```

You do not need to manually edit the .csproj if you use the dotnet add package command.
